...
//
	...
	private double getDroneDistance(Line2D s1, Point2D p) {
		Point2D p1 = GeometryTools2D.getClosestPointOnSegment(s1, p);
		return GeometryTools2D.getDistance(p1, p);
	}

	private double getDroneDistance(Line2D s1, Line2D s2) {
		double d1 = getDroneDistance(s1, s2.getOrigin());
		double d2 = getDroneDistance(s1, s2.getEndPoint());
		double d3 = getDroneDistance(s2, s1.getOrigin());
		double d4 = getDroneDistance(s2, s1.getEndPoint());
		d1 = Math.min(d1, d2);
		d2 = Math.min(d4, d3);
