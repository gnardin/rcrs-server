...
public class TrafficSimulator extends StandardSimulator implements GUIComponent {
  ...
  protected void processCommands(KSCommands c, ChangeSet changes) {
    ...
    for (TrafficAgent agent : manager.getAgents()) {
      ...
      // loaded or unloaded
      Robot robot = agent.getRobot();
      if (!agent.isMobile()) {
        robot.undefinePositionHistory();
        robot.setTravelDistance(0);
      ...
      if (location != null) {
        robot.setPosition(location.getArea().getID());
        changes.addChange(robot, robot.getPositionProperty());
      }
      robot.setX((int) x);
      robot.setY((int) y);
      robot.setPositionHistory(historyArray);
      robot.setTravelDistance((int) agent.getTravelDistance());
      changes.addChange(robot, robot.getXProperty());
  ...
  private void handleFly(AKFly fly) {
    ...
    Entity currentEntity = model.getEntity(robot.getPosition());
    ...
    for(EntityID next : list) {
      ...
      steps.addAll(getPathElements2(robot, currentArea, lastEdge, nextArea, edge));
  ...
  private Collection<? extends PathElement> getPathElements2(Robot robot, Area lastArea, Edge lastEdge, Area nextArea,
    ...
    Point2D start;
    if (lastEdge == null)
      start = new Point2D(robot.getX(), robot.getY());
    else
